...
aws lambda invoke \
    ...
    --region eu-west-2 \
    --payload '{"region": "eu-west-2","bucket": "automatictester.co.uk-lightning-aws-lambda","mode": "report","jmeterCsv": "csv/jmeter/10_transactions.csv"}' \
    response.json \
    &> /dev/null

S3_OBJECT=`cat response.json | jq -r '.jmeterReport'`
